...
def train(model, optimizer, epoch, data_src, data_tar=None):
    ...
    #for batch_id, batch in enumerate(iterate_minibatches(data_src[0], data_src[1], BATCH_SIZE)):
        ...
        if inputs.shape[0] != 100:
            continue
        #x_tar, y_target = x_tar.view(-1, 1, 24,3).to(DEVICE), y_target.to(DEVICE)

        #inputs, targets = batch #(100,24,3)  (100,)
        #inputs,targets = torch.from_numpy(inputs).view(-1, 1, 24,3).to(DEVICE), torch.from_numpy(targets).to(DEVICE).type(torch.cuda.FloatTensor)
        #targets = targets.type(torch.cuda.LongTensor) 

        model.train()
        y_src = model(inputs)
        print(y_src.shape) # [100, 18]
...
X_test, y_test = opp_sliding_window(X_test, y_test, SLIDING_WINDOW_LENGTH, SLIDING_WINDOW_STEP)  #(9894, 24, 3) 
